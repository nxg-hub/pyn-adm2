...
const bettingTransactions = [
  ...
  {
    id: "BET-003",
    game: "Football - Champions League",
    provider: "Betfair",
    amount: "$50.00",
    method: "Wallet",
    date: "2024-04-12",
...
export default function BettingAndLotteryPage() {
  ...
  const getActionItems = (txn) => {
    ...
    if (txn.status === "won" && hasPermission("viewFinancialReports")) {
      actions.push({
    ...
    <>
      <Card>
        <CardHeader>
          ...
          <CardDescription>View and manage betting and lottery transactions.</CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
            ...
            <TableBody>
              {bettingTransactions.map((txn) => (
                <TableRow key={txn.id}>
                  <TableCell>{txn.id}</TableCell>
                  <TableCell>{txn.game}</TableCell>
          ...
          {selectedTransaction && (
            <div className="grid gap-4 py-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div><h3 className="font-medium text-sm">Game</h3><p>{selectedTransaction.game}</p></div>
