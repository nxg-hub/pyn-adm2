...
export function ScanToPayTable() {
  ...
  const filtered = scanToPayData.filter(
  ...
  const getActionItems = (txn) => {
    ...
    if (txn.status === "paid" && hasPermission("viewScanToPayReports")) {
      actions.push({
    ...
    <>
      <Card>
        <CardHeader>
          ...
          <CardDescription>Manage and review all scan to pay activity.</CardDescription>
              ...
              {paginatedItems.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={7} className="text-center text-muted-foreground">
                    No scan to pay transactions for this status.
                  </TableCell>
                </TableRow>
              ) : (
      ...
      {flagTxn && (
        <ReasonPromptDialog
          title="Flag Suspicious Scan to Pay"
